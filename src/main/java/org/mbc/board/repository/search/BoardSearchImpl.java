...
public class BoardSearchImpl extends QuerydslRepositorySupport implements BoardSearch {
    ...
    public Page<Board> search1(Pageable pageable) {
        ...
        JPQLQuery<Board> query = from(board);//select * from board 완성

        BooleanBuilder booleanBuilder = new BooleanBuilder();
        //다중조건일 때 연산자 공식에 의해서 특수 기호가 먼저 계산 될 때가 있음
        //()를 사용하면 선행 되기 때문에 BooleanBuilder가 이 역활을 한다
        ...
        query.where(board.bno.gt(0L));//pk를 이용해서 빠른 검색 where이 추가되면 and조건
        //where (title like 11 or content like 11) and bno > 0

        //페이징 처리용 코드
        this.getQuerydsl().applyPagination(pageable, query);
        List<Board> list = query.fetch();//쿼리문 실행해서 list에 담아라
        long count = query.fetchCount();//검색 후에 게시물 수 파악용
        return null;
    }
    ...
    public Page<Board> searchAll(String[] types, String keyword, Pageable pageable) {
        ...
        if( (types != null && types.length > 0) && keyword != null){
            //검색 조건(제목,내용,이름 값)과 키워드(검색어)가 있다면
        ...
        long count = query.fetchCount();//검색된 게시물 수를 count에 넣음
        
        //Page<> 클래스는 3가지의 리턴 타입을 만들어줌
