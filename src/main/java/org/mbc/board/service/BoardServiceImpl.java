...
@Transactional//commit용(여러개의 테이블이 조합될 때 해결 역활)
public class BoardServiceImpl implements BoardService{
    ...
    public BoardDTO readOne(Long bno) {
        ...
        Board board = result.orElseThrow();//정상 값이 나오면 엔티티로 받음

        BoardDTO boardDTO = modelMapper.map(board, BoardDTO.class);
        //모델 메퍼를 이용해서 엔티티로 나온 board를 dto로 변환
    ...
    public void modify(BoardDTO boardDTO) {
        ...
        boardRepository.save(board);//데이터 베이스에 pk가 있으면 update, 없으면 insert
    }
