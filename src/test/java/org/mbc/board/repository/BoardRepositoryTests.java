...
public class BoardRepositoryTests {
    ...
    public void testSelect(){
        ...
        //널값이 나올 경우를 대비한 객체
        //                                     .finById(Bno); -> selsct * from board where bno = bno;
        //Hibernate:
        //    select
        //        b1_0.bno,
        //        b1_0.content,
    ...
    public void testUpdate(){
        ...
        Optional<Board> result = boardRepository.findById(bno);//bno을 찾아서 result에 넣는다
        Board board = result.orElseThrow();//가져온 값이 있으면 board타입에 객체에 넣는다
        board.change("수정테스트 제목","수정테스트 내용");//제목과 내용만 수정할 수 있는 메서드
        boardRepository.save(board);//.save메서드는 pk값이 없으면 insert,있으면 upadte함
        //Hibernate:
        //    select
        //        b1_0.bno,
        //        b1_0.content,
    ...
    public void testPaging(){
        ...
        //전체리스트에 페이징과 정렬 기법도 추가

        Pageable pageable = PageRequest.of(0,10, Sort.by("bno").descending());
        //                                  시작번호   페이지당 데이터개수   번호를 기준으로 내림차순 정렬
        //Hibernate:
        //    select
        //        b1_0.bno,
        //        b1_0.content,
        ...
        //1장에 종이에 board객체를 가지고 있는 결과는 result에 넣는다
        //page클래스는 다음페이지 존재 여부, 이전페이지 존재여부, 전체 데이터 개수, 등 계산을 함
